	...
	function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $post_min > 0 || $post_max > 0 ) {
			...
			if ( $post_min ) {
				...
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="#" data-filter="'.urldecode(esc_url($link)).'" data-link="'.urldecode(esc_url($link)).'">' . __( 'Min', 'woocommerce' ) . ' ' . wc_price( $post_min ) . '</a></li>';
			}

			if ( $post_max ) {
				$link = esc_url( remove_query_arg( 'max_price' ) );
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'woocommerce' ) . '"  href="#" data-filter="'.urldecode(esc_url($link)).'" data-link="'.urldecode(esc_url($link)).'">' . __( 'Max', 'woocommerce' ) . ' ' . wc_price( $post_max ) . '</a></li>';
			}
